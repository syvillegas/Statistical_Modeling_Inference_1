...
lasso.bic_mod <- function(y,x,extended=FALSE, family='gaussian') {
  #Select model in LASSO path with best BIC (using LASSO regression estimates)
  #Input
  # - y: vector with response variable
  # - x: design matrix
...
kfoldCV.mle_mod <- function(y,x,K=10,seed, family = "gaussian", predict_type = "link") {
  ## Perform K-fold cross-validation for least-squares regression estimate
  ## Input
  ## - y: response
  ## - x: data.frame with predictors, intercept should not be present
...
kfoldCV.lasso_mod <- function(y,x,K=10,seed,criterion='cv', family = 'gaussian', predict_type = "link") {
  ## Perform K-fold cross-validation for LASSO regression estimate (lambda set either via cross-val or BIC or EBIC)
  ## Input
  ## - y: response
  ## - x: data.frame with predictors, intercept should not be present
  ...
  if (ncol(x)>0) {  #if there are some covariates
    for (k in 1:K) {
      ...
      if (criterion=='cv') {
        ...
        fit <- lasso.bic_mod(y=y[!sel],x=x[!sel,,drop=FALSE], family = family, extended = FALSE)
        ...
        fit <- lasso.bic_mod(y=y[!sel],x=x[!sel,,drop=FALSE], family = family, extended = TRUE)
        model <- glmnet(x=x[!sel,,drop=FALSE],y=y[!sel],lambda = fit$lambda.opt,family=family)
        pred[sel] <- predict(model, newx= x[sel,,drop=FALSE], type = predict_type)
      } else { stop("method.lambda not implemented") }
      cat(".")
...
simdata.cs <- function(seed,n,theta,phi=1,rho) {
    #Simulate n observations from a linear model y= X %*% theta + e,
    #where e ~ N(0,phi)
    #and the rows in X ~ Normal(0,Sigma) and Sigma is compound symmetric 
    #(Sigma[i,i]=1, Sigma[i,j]=rho)
    #Input
